...
export const SpinWheel: React.FC = () => {
  ...
  const onAdComplete = () => {
      setShowAd(false);
      // Optional: reset wheel rotation here if desired
  };
  ...
  return (
    <div className="h-[calc(100dvh-160px)] w-full flex flex-col items-center justify-between py-4 px-4 relative overflow-hidden">
        ...
        <div className="text-center space-y-1 mt-6">
            <h1 className="text-3xl font-black text-white tracking-tighter">SPIN & WIN</h1>
            <p className="text-blue-500 text-[9px] font-black uppercase tracking-[0.3em]">Fortune favors the bold</p>
        </div>
        ...
        <div className="w-full max-w-xs pb-4">
            <button 
                onClick={handleSpin}
                disabled={spinning}
                className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-800 text-white font-black text-xs uppercase tracking-[0.3em] py-5 rounded-[2rem] shadow-2xl shadow-blue-900/40 active:scale-95 transition-all border border-blue-400/10"
            >
        ...
        {result && (
            <div className="fixed inset-0 z-[100] bg-black/90 backdrop-blur-xl flex items-center justify-center p-6 animate-in fade-in duration-300">
                <div className="bg-[#1e293b] w-full max-w-sm rounded-[2.5rem] border border-white/5 p-8 text-center space-y-6 animate-in zoom-in duration-500 shadow-2xl">
                    ...
                    <div className="space-y-1">
                        <h2 className="text-2xl font-black text-white tracking-tight uppercase">JACKPOT!</h2>
                        <p className="text-gray-500 text-[9px] font-black uppercase tracking-[0.3em]">Points Awarded Successfully</p>
                    </div>
                    <div className="bg-white/5 py-4 rounded-2xl border border-white/5">
                        <span className="text-4xl font-black text-blue-500 tracking-tighter">+{result.reward}</span>
                        <span className="text-xs font-black text-blue-400/60 uppercase ml-2 italic">Pts</span>
                    </div>
                    <button 
                        onClick={handleCollect} 
                        className="w-full bg-blue-600 py-5 rounded-2xl font-black text-xs uppercase tracking-widest text-white shadow-xl shadow-blue-900/30 active:scale-95 transition-all"
                    >
                        Collect & Continue
                    </button>
                </div>
